...
class MultiSelectDropDown extends React.PureComponent {
  ...
  handleAllChecked(event) {
    ...
    let eventChecked = event.target ? event.target.checked : event;
    itemList.forEach(item => (item.isChecked = eventChecked))

    this.setState({
      itemList,
      selectAllChecked: eventChecked,
      selectedItems: eventChecked ? itemList : []
    })

    this.props.selectedItems(eventChecked ? itemList : [])
  }
        ...
        {showMenu && (
          <div className='drop-down__menu-box'>
            ...
            {enableSearchFilter && <div className='list-filter'>
              ...
              <span className='c-clear' onClick={() => this.handleSearchBoxClear()}>
              ...
              <input ref={el => this.inputTitle = el} autoFocus onChange={(e) => this.handleFilter(e.target.value)} className='c-input' type='text' placeholder={placeHolderText} />
            </div>}
